...
public class MorseDecoder {
	...
	String decode(String morse) {
		String[] morseStrings = morse.split(" ");
		String word = "";
		for (int i = 0; i < morseStrings.length; i++) {
			word += decodeCharacter(morseStrings[i]);
		}

		return word;
	}

	String decodeCharacter(String morseCharacter) {
		System.out.println("/"+morseCharacter+"/");
		Node<MorseCode> root = mcTree.getRoot();
		Node<MorseCode> nodeQuery = recursiveSearch(root, morseCharacter);
		if (nodeQuery == null) {
	...
	Node<MorseCode> recursiveSearch(Node<MorseCode> current, String morse) {
		MorseCode value = new MorseCode("", morse);
